...
class Burger:
    ...
    def customize_burger(self, bun=None, cheese=None, patty=None, onion=None, pickle=None, lettuce=None, spread=None, tomato=None):
        if bun: self.bun = bun
        if cheese: self.cheese = cheese
        if patty: self.patty = patty
        if onion: self.onion = onion
        if pickle: self.pickle = pickle
        if lettuce: self.lettuce = lettuce
        if spread: self.spread = spread
        if tomato: self.tomato = tomato
...
class Order:
    ...
    def store_order(self, filename):
        ...
        with open(filename, 'w') as file:
            ...
            file.write(f"Griled Onion: {self.burger.griled_onion}\n")
            file.write(f"Raw Onion: {self.burger.griled_onion}\n")
...
class KioskApp:
    ...
    def create_customize_widgets(self):
        ...
        for idx, (label_text, var_name, options, var) in enumerate(ingredients):
            ...
            for option in options:
                tk.Button(frame, text=str(option), 
                          command=lambda b=option, v=var_name, f=frame: [getattr(self, f"{v}_var").set(b), self.update_button_color(f, b)],
                          bg='lightgray', activebackground='lightblue', relief=tk.RAISED).pack(side=tk.LEFT)
        ...
        presets = [
            ("Ham Burger", {'bun': 'medium', 'cheese': 0, 'patty': 1, 'pickle': 3, 'lettuce': 3, 'spread': 3, 'griled_onion': 0, 'raw_onion': 0}),
            ("Double Double", {'bun': 'medium', 'cheese': 2, 'patty': 2, 'pickle': 3, 'lettuce': 3, 'spread': 3, 'griled_onion': 0, 'raw_onion': 0}),
            ("Cheese Burger", {'bun': 'medium', 'cheese': 1, 'patty': 1, 'pickle': 3, 'lettuce': 3, 'spread': 3, 'griled_onion': 0, 'raw_onion': 0})
        ]
    ...
    def apply_preset(self, config):
        for key, value in config.items():
            ...
            if key in ['bun', 'cheese', 'patty', 'pickle', 'lettuce', 'spread', 'griled_onion', 'raw_onion']:
                frame_name = f"{key}_frame"
                if hasattr(self, frame_name):
                    self.update_button_color(getattr(self, frame_name), value)

    def update_button_color(self, frame, selected_value):
        for widget in frame.winfo_children():
            if widget.cget('text') == str(selected_value):
                widget.config(bg='lightblue' if widget.cget('bg') == 'lightgray' else 'lightgray')
            else:
                widget.config(bg='lightgray')

    def place_order(self):
        # Store the order as a text file with current time + user id as filename
    ...
    def create_order_widgets(self):
        ...
        tk.Button(self.root, text="Load Preferred Order", command=self.load_preferred_order).grid(row=1, column=0, columnspan=2)
